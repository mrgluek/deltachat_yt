...
- **Track & Chapter Slicing:** Automatically detects tracklists and chapters from video metadata or descriptions (e.g. YouTube chapter lists and inline timestamps). Proposes track slicing (Track 1, Track 2, etc.) with exact track boundaries instead of arbitrary chunks, uses `yt-dlp --download-sections` for rapid range downloads without downloading the whole stream or re-encoding, and embeds track title, album, artist, and track numbers into tags.
...
- **Video Downloads & Automatic Chunking (`/yt`):** Downloads video in MP4 with automatic multi-resolution fallback. Videos longer than 10 minutes are automatically offered in 10-minute chunks (`/yt_ID_0_600`) or chapter tracks to guarantee delivery within 50 MB size limits, with clickable `▶️ Next track/chunk` links included in each delivered file's caption.
- **Audio Downloads (`/ytm`):** Extracts audio as high-quality Opus with embedded metadata tags (Title, Artist, Album, Track Number, Cover Art, embedded plain-text `LYRICS`) and the original video URL embedded in the file's description/comment metadata tag. Direct stream-copied without re-encoding to preserve original audio quality.
...
- **Navidrome / Subsonic Integration (`/ytms`):** Admin command to save downloaded tagged audio directly into a Navidrome music directory organized by Artist/Album/Title along with synchronized `.lrc` companion files for karaoke lyrics display, and immediately trigger a Subsonic REST API library scan (`startScan.view`). Seamlessly handles sliced tracks from long DJ sets and compilations.
