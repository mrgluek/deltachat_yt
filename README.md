...
- **Video Downloads & Automatic Chunking (`/yt`):** Downloads video in MP4 with automatic multi-resolution fallback. Videos longer than 10 minutes are automatically offered in 10-minute chunks (`/yt_ID_0_600`) to guarantee delivery within 30 MB size limits, with clickable `▶️ Next chunk` links included in each delivered file's caption.
