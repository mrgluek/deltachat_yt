...
_user_rate_limits_lock = threading.Lock()
...
_processed_msg_ids: collections.OrderedDict[str, None] = collections.OrderedDict()
...
def _is_rate_limited(bot, accid, from_id) -> bool:
    ...
    if _is_dc_admin(bot, accid, from_id):
        return False
    now = time.time()
    with _user_rate_limits_lock:
        last = _user_rate_limits.get(from_id, 0)
        if now - last < RATE_LIMIT_SECONDS:
            return True
        _user_rate_limits[from_id] = now
        if len(_user_rate_limits) > 500:
            cutoff = now - max(60.0, RATE_LIMIT_SECONDS * 2)
            expired = [uid for uid, t in _user_rate_limits.items() if t < cutoff]
            for uid in expired:
...
async def _download_audio(video_id: str, output_dir: str, duration: int, start_time: int = None, end_time: int = None, use_cookies: bool = True, custom_proxy: str = None, player_client: str = None, for_slicing: bool = False) -> tuple[str | None, dict | None, str | None]:
    ...
    if start_time is not None or end_time is not None:
        ...
        if cached_base and os.path.exists(cached_base):
            ...
            trim_cmd = ["ffmpeg", "-y", "-nostdin", "-ss", str(start_time or 0), "-i", cached_base]
            if end_time is not None:
                trim_duration = end_time - (start_time or 0)
                trim_cmd.extend(["-t", str(trim_duration)])
            trim_cmd.extend(["-c", "copy", sliced_path])
            ...
            if p_trim.returncode != 0 or not os.path.exists(sliced_path) or os.path.getsize(sliced_path) == 0:
                ...
                fb_cmd = ["ffmpeg", "-y", "-nostdin", "-ss", str(start_time or 0), "-i", cached_base]
                if end_time is not None:
                    trim_duration = end_time - (start_time or 0)
            ...
            if os.path.exists(sliced_path) and os.path.getsize(sliced_path) > 0:
                ...
                err_msg = _clean_ffmpeg_error(fb_err or trim_err)
                logger.warning(f"ffmpeg slicing failed for {video_id} on base {cached_base}: {err_msg}")
                try:
                    os.remove(cached_base)
                    logger.info(f"Purged unsliceable cached base: {cached_base}")
                except Exception:
                    pass
...
def transports_command(bot, accid, event):
    ...
    try:
        ...
        _send(bot, accid, msg.chat_id, "❌ Failed to list transports. Check server logs for details.")
        return
...
import collections
_message_failover_attempts: collections.OrderedDict[int, dict] = collections.OrderedDict()
_message_failover_lock = threading.Lock()
...
def on_msg_failed(bot, accid, event):
    ...
    try:
        with _message_failover_lock:
            while len(_message_failover_attempts) > 1000:
                _message_failover_attempts.popitem(last=False)
        ...
        try:
            ...
            if not msg_snapshot:
                return
            state_val = msg_snapshot.get('state') if isinstance(msg_snapshot, dict) else getattr(msg_snapshot, 'state', None)
            if state_val != 24:
                return
        except Exception:
            return
            ...
            try:
                ...
                with resilient_lock:
                    try:
                        ...
                        # Always restore the initial primary transport address under the lock!
                        try:
                ...
                if "e2e encryption" in err_str or "encryption" in err_str:
                    ...
                    try:
                        with _message_failover_lock:
                            if msg_id in _message_failover_attempts:
